...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Where(s => s.EpisodeCount <= 99).ToList();
        }
        ...
        {
            return shows.Where(s => s.Genres.Contains("Drama"))
        ...
        {
            return shows.Where(s => s.ImdbRating > 7 && s.Genres.Contains("Crime"))
            .Count();
        }
        ...
        {
            return shows.OrderByDescending(s => s.Name.Length)
            .FirstOrDefault();
        }
        ...
        {
            IEnumerable<string> mostShows = shows.Take(shows.Count() -2).Select(s => s.Name);
            IEnumerable<string> lastTwo = shows.Skip(shows.Count() -2).Select(s => s.Name);

            string showsStrungByName = String.Join(", ", mostShows);
            showsStrungByName += $", {String.Join(", and ", lastTwo)}";


            return showsStrungByName;
        }
